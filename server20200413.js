...
app.get("/index", function(req, res, next) {
    ...
    get_group_info_FromDB(function (err, group_result) {
        ...
        con.query(sql, function (err, result, fields) {
            ...
            console.log(result);
            res.render('index', {result: result, user: current_user, group_result: group_result}); 
         });
  });
});
...
app.get("/show_your_user_information", function(req, res, next) {
    ...
    con.query(sql, function (err, result, fields) {
        ...
        con.query(sqlr, function (err, rating_result, fields) {
                ...
                res.render('user_information_display', {result: result[0], user: current_user, rate: rating_result[0]['AVG(rating)']}); 
        });
    });
});
...
app.get("/show_post", function(req, res, next) {
    ...
    con.query(sqlc, function (err, comment_result, fields) {
        ...
        con.query(sqlp, function (err, post_result, fields) {
            ...
            res.render('postdisplay', {result: post_result[0], all_result: post_result, comment: comment_result}); 
        });
        //console.log("post_id: "+result[0].post_id);
...
app.get("/enter_room", function(req, res, next) {
    ...
    var q = url.parse(req.url, true);
	var qdata = q.query;
	console.log(qdata.enter_post_id);
	var sql = "Select * FROM post, post_to_join WHERE post.post_id = post_to_join.post_id AND post.post_id = '"+qdata.enter_post_id+"'";
	console.log(sql);
    con.query(sql, function (err, result, fields) {
    if (err) throw err;
    console.log(result);
    console.log("post_id: "+result[0].post_id);
    res.render('room_display', {result: result[0], all_result: result}); 
  });
});
...
app.get("/rate_user", function(req, res, next) {
    ...
    console.log(qdata.rate_of_user[i]);
